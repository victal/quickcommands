    ...
    .then(items => {
      ...
      for (let item of items) {
        if (item.type === 'bookmark' && item.title.trim().length > 0) {
          ...
          let allowedUrl =
            !item.url.toLowerCase().startsWith('place') &&
            !item.url.toLowerCase().startsWith('about')
          if (allowedUrl) {
