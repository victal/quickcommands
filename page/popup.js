...
async function startUp() {
    ...
    updateTheme().then(async () => {
        return browser.history.deleteUrl({url: url}).then(async () => {
            console.debug("Extension page removed from history");
            let enabledLists = await getLists();
            let lists = [];
            if (enabledLists.includes("Tabs")) { lists.push(new TabList("Tabs", updateTabs)) }
            if (enabledLists.includes("History")) { lists.push(new TabList("History", updateHistoryTabs)) }
            if (enabledLists.includes("Bookmarks")) { lists.push(new TabList("Bookmarks", updateBookmarks)) }
